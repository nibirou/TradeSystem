...
if [[ -z "${model_summary_json}" ]]; then
  echo "Usage: bash scripts/v2/run_strategy7_v2_07_load_custom_from_summary.sh --model-summary-json <path>" >&2
  exit 2
fi

output_dir="${repo_root}/Strategy7/outputs/smoke_v2/run_strategy7_07_load_custom_summary"
mkdir -p "${output_dir}"

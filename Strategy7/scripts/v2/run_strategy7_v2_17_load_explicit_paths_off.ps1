...
$outputDir = Join-Path $repoRoot "Strategy7\outputs\smoke_v2\run_strategy7_17_load_explicit_paths_off"
...
conda run -n env_quant --no-capture-output python .\Strategy7\run_strategy7.py `
  ...
  --model-run-mode load `
  --load-fe-mode off `
  --stock-model-path $StockModelPath `
  --timing-model-path $TimingModelPath `
  --portfolio-model-path $PortfolioModelPath `

...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --execution-model-path)
      execution_model_path="$2"; shift 2 ;;
    *)
      echo "Unknown argument: $1" >&2; exit 2 ;;
  esac
done

if [[ -z "${timing_model_path}" || -z "${execution_model_path}" ]]; then
  echo "Usage: bash scripts/v2/run_strategy7_v2_31_mixed_component_modes.sh --timing-model-path <path> --execution-model-path <path>" >&2
  exit 2
fi

...
if [[ -z "${model_summary_json}" ]]; then
  echo "Usage: bash scripts/v2/run_strategy7_v2_21_load_allmarket_bottom_launch_10d.sh --model-summary-json <path> [--data-root auto] [--max-files 0]" >&2
  exit 2
fi

output_dir="${repo_root}/Strategy7/outputs/smoke_v2/run_strategy7_21_load_allmarket_bottom_launch_10d"
mkdir -p "${output_dir}"

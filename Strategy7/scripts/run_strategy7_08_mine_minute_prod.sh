#!/usr/bin/env bash
# bash /workspace/Quant/TradeSystem/Strategy7/scripts/run_strategy7_08_mine_minute_prod.sh
set -euo pipefail
...
python3 ./Strategy7/run_factor_mining.py \
  ...
  --catalog-path auto \
  --save-format parquet \
  --min-abs-ic-mean 0.008 \
  --min-ic-win-rate 0.51 \
  --min-ic-ir 0.08 \
  --min-long-excess-annualized 0.008 \
  --min-long-sharpe 0.20 \

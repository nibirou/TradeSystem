...
3. 条件生效参数说明：  
   - 某些参数仅在对应模型/框架下生效（如 `launch_boost_*`、`fgcl_*`、`dafat_*`、`timesnet_*`、`stockformer_*`、`gp_*`、`ml_*`），这是设计行为，不是空转。
...
3. 选股模型参数（`stock-model-type` + `launch_boost_*` + `fgcl_*` + `dafat_*` + `timesnet_*` + `stockformer_*` + 决策树参数）
...
1. 条件生效不等于未使用：  
`launch_boost_*` 在非 `launch_boost` 下不会生效，`fgcl_*` 在非 `factor_gcl` 下不会生效，`timesnet_*` 在非 `dfq_timesnet` 下不会生效，`stockformer_*` 在非 `stockformer` 下不会生效，`gp_*` 在非 `gplearn_symbolic_alpha` 下不会生效，这是预期设计。
...
4. `models/loading.py` 负责：
   ...
   - 加载内置四类模型（含 tree/launch_boost/factor_gcl/dafat/dfq_timesnet/stockformer + timing/portfolio/execution）

"""Minute parametric plus mining model."""
...
def run_minute_parametric_plus_model(
    *,
    panel: pd.DataFrame,
    ...
    log_progress(
        f"Start minute-plus evolution: framework={framework}, feature_pool={len(fields)}, "
    ...
    for gen in range(generations):
        ...
        log_progress(
            (
                f"[minute_plus][gen={gen:02d}] best_score={best['score']:.4f} "
    ...
    log_progress(f"Minute-plus evolution finished: candidate_count={len(results)}.", module="mining")

...
def resample_intraday(minute_df: pd.DataFrame, freq: str) -> pd.DataFrame:
    ...
    rs = (
        m.groupby(
            ["code", pd.Grouper(key="datetime", freq=rule, label="right", closed="right")],
            sort=True,
            observed=True,
        )
        .agg(agg_map)
    ...
    out_cols = ["datetime", "open", "high", "low", "close", "volume", "amount", "code", "date"]
    return rs[out_cols].sort_values(["code", "datetime"]).reset_index(drop=True)
...
def resample_daily_to_period(daily_df: pd.DataFrame, freq: str) -> pd.DataFrame:
    ...
    d = d.dropna(subset=["code", "date"]).sort_values(["code", "date"])
    rs = (
        d.groupby(["code", pd.Grouper(key="date", freq=rule)], sort=True, observed=True)
        .agg(
            {
                "open": "first",
                "high": "max",
    ...
    out_cols = ["date", "open", "high", "low", "close", "volume", "amount", "turn", "code"]
    out = rs[out_cols].sort_values(["code", "date"]).reset_index(drop=True)
...
def _aggregate_source_to_target(
    ...
    *,
    ...
    if target_freq in {"W", "M"}:
        ...
        daily_src = daily_src.sort_values(["code", "date"])
        rs = daily_src.groupby(["code", pd.Grouper(key="date", freq=rule)], sort=True, observed=True).agg(agg_map)
        rs = _flatten_agg_columns(rs, prefix=prefix).reset_index()
        if rs.empty:
            return pd.DataFrame()
        data_cols = [c for c in rs.columns if c not in {"date", "code"}]
        out = rs[["date", *data_cols, "code"]]
        return out.sort_values(["code", "date"]).reset_index(drop=True)
...
def add_generic_micro_structure_features(
    ...
    *,
    ...
    out["ma_6"] = _rolling_mean(g["close"], 6)
    out["ma_12"] = _rolling_mean(g["close"], 12)
    out["ma_gap_6"] = out["close"] / (out["ma_6"] + 1e-12) - 1.0
    out["ma_gap_12"] = out["close"] / (out["ma_12"] + 1e-12) - 1.0

    out["rv_12"] = _rolling_std(g["ret_1"], 12)
    out["range_norm"] = (out["high"] - out["low"]) / (out["close"].abs() + 1e-12)
    out["amount_ma12"] = _rolling_mean(g["amount"], 12)

...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train-start", type=str, default="2025-09-01", help="训练开始日期。")
    parser.add_argument("--train-end", type=str, default="2025-09-30", help="训练结束日期。")
    parser.add_argument("--test-start", type=str, default="2025-10-01", help="回测开始日期（信号日）。")
    ...
    parser.add_argument("--file-format", type=str, choices=["auto", "parquet", "csv"], default="auto", help="读取文件格式偏好。")
...
def load_hs300_data(
    ...
    keys = list_symbol_keys(daily_dir, file_format=file_format)
    ...
    if not daily_frames:
        raise RuntimeError("未加载到日频数据，请检查路径和日期范围。")
    if not minute_frames:
        raise RuntimeError("未加载到5分钟数据，请检查路径和日期范围。")
...
def add_label(panel: pd.DataFrame, horizon: int) -> pd.DataFrame:
    """构建监督学习标签。
    ...
    - 入场日: T+1
    - 出场日: T+1+horizon
    - 标签收益: exit_close_ref / entry_close_ref - 1
    """
    out = panel.copy()
    g = out.groupby("code")

    # T 日信号在收盘后产生，因此最早只能在 T+1 入场。
    out["entry_date"] = g["date"].shift(-1)
    # 持有 horizon 个交易日后，在 T+1+horizon 出场。
    out["exit_date"] = g["date"].shift(-(horizon + 1))

    # 标签收益使用日频收盘“参考收益”，仅用于监督学习，不等于回测执行收益。
    out["entry_close_ref"] = g["close"].shift(-1)
    out["exit_close_ref"] = g["close"].shift(-(horizon + 1))
    out["future_ret_n"] = out["exit_close_ref"] / (out["entry_close_ref"] + EPS) - 1.0
...
def run_backtest(
    ...
    回测口径：
    - 每 horizon 个交易日调仓一次（非重叠持有）
    - 当期候选池按 pred_prob_up 过滤并取 top_k
    - 收益由执行方案价格与交易成本共同决定
    """
    ...
    # 非重叠持有：每 horizon 个交易日调仓一次。
    for idx in range(0, len(rebalance_dates), horizon):
        dt = rebalance_dates[idx]
        day_all = pred_df[(pred_df["date"] == dt)].copy()
        day_all = day_all.dropna(subset=["entry_price", "exit_price", "net_trade_ret", "future_ret_n"])

        # 基准收益为当天所有样本的平均净收益率（即使不选股也能获得的收益），以此计算策略的超额收益。
        bench_ret = float(day_all["net_trade_ret"].mean()) if not day_all.empty else 0.0
        bench_size = int(len(day_all))

        # 先按阈值过滤，再按上涨概率排序取 top_k。
    ...
    if trades_df.empty:
        empty_curve = pd.DataFrame(columns=["trade_date", "strategy_ret", "benchmark_ret", "excess_ret", "strategy_net", "benchmark_net", "excess_net", "strategy_cum_return", "benchmark_cum_return", "excess_cum_return"])
        summary = {
    ...
    # 计算净值曲线与累计收益率曲线。
    # 净值曲线 = (1 + 分期收益率) 的累积乘积，初始值为 1.0。
    # cumprod() 用法：对于序列 [r1, r2, r3]，计算 (1+r1) * (1+r2) * (1+r3)，得到每个时间点的净值水平。
    trades_df["strategy_net"] = (1.0 + trades_df["strategy_ret"]).cumprod()
    trades_df["benchmark_net"] = (1.0 + trades_df["benchmark_ret"]).cumprod()
    trades_df["excess_net"] = (1.0 + trades_df["excess_ret"]).cumprod()
    
    # 累计收益率 = 净值 - 1
    trades_df["strategy_cum_return"] = trades_df["strategy_net"] - 1.0
    trades_df["benchmark_cum_return"] = trades_df["benchmark_net"] - 1.0
    trades_df["excess_cum_return"] = trades_df["excess_net"] - 1.0

    strategy_stats = compute_return_stats(trades_df["strategy_ret"], horizon=horizon)
    benchmark_stats = compute_return_stats(trades_df["benchmark_ret"], horizon=horizon)
    excess_stats = compute_return_stats(trades_df["excess_ret"], horizon=horizon)
    ...
    curve_df = trades_df[curve_cols].copy()
    return trades_df, positions_df, curve_df, summary, pred_df


def plot_backtest_curves(curve_df: pd.DataFrame, output_png: Path, title: str) -> bool:
    """绘制并保存分期收益曲线与净值曲线。"""
    ...
    axes[0].plot(dt, curve_df["strategy_ret"], label="Strategy Period Return", linewidth=1.2)
    axes[0].plot(dt, curve_df["benchmark_ret"], label="Benchmark Period Return", linewidth=1.2)
    axes[0].axhline(0.0, color="gray", linewidth=0.8, linestyle="--")
    axes[0].set_ylabel("Period Return")
    axes[0].legend(loc="best")
    axes[0].grid(alpha=0.2)

    axes[1].plot(dt, curve_df["strategy_net"], label="Strategy Net Value", linewidth=1.6)
    axes[1].plot(dt, curve_df["benchmark_net"], label="Benchmark Net Value", linewidth=1.6)
    axes[1].plot(dt, curve_df["excess_net"], label="Excess Net Value", linewidth=1.4)
...
def main() -> None:
    ...
    if not (train_start <= train_end < test_start <= test_end):
        raise ValueError("日期要求: train_start <= train_end < test_start <= test_end")
    ...
    # 因子计算与回测通常需要比训练/测试窗口更长的历史数据，尤其是当 lookback_days 和 horizon 较大时。
    load_start = train_start - pd.Timedelta(days=int(args.lookback_days))
    # 回测标签的 exit_date 最晚为 test_end，因此 load_end 需要覆盖到 test_end + horizon。
    ...
    panel = add_label(panel, horizon=args.horizon)
    ...
    if train_df.empty:
        raise RuntimeError("训练集为空，请扩大训练区间或检查数据。")
    if test_df.empty:
        raise RuntimeError("测试集为空，请扩大测试区间或检查数据。")
    ...
    price_table = minute_daily_feat[["code", "date", "px_open5", "px_vwap30", "px_twap_last30"]].copy()
    ...
    plot_path = output_dir / f"backtest_curve_{run_tag}.png"
    ...
    plot_saved = plot_backtest_curves(
        curve_df,
        output_png=plot_path,
        title=f"Strategy vs Benchmark ({args.execution_scheme})",
    )
    ...
    summary = {
        "config": {
            ...
            "note_no_future_leakage": (
                "因子仅用信号日及历史数据；标签采用T+1入场到T+1+horizon出场参考收益；"
                "训练/测试均限制target_date不越界；买卖价仅使用各自执行日内可观测5分钟数据。"
            ),
        },
        "sample_count": {
        ...
        "outputs": {
            ...
            "backtest_plot_png": str(plot_path) if plot_saved else "未生成（matplotlib不可用或曲线为空）",
        },
    }
    ...
    if plot_saved:
        ...
        print("Backtest plot skipped    : matplotlib不可用或曲线为空")
